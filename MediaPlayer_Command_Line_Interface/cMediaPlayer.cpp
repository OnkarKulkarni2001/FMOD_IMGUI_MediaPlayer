...
void cMediaPlayer::StartMediaPlayer() {
    ...
    audioManager.loadAssets();      // loading assets
        ...
        if (_kbhit()) {
            ...
            if (key == 'P' || key == 'p' ) {
                ...
                if (indexOfSong >= 0 && indexOfSong < audioManager.sounds.size()) {
                    ...
                    PlayAudio(indexOfSong);     // Calling play audio function by passing index by value
                }
                else {
...
void cMediaPlayer::StopMediaPlayer() {
    ...
    cin >> decision;            // Checking if user really wants to quit or not
...
void cMediaPlayer::DisplayProgressBar() {
    ...
    cout << "Press H key to hide progress bar" << endl;
    while (true) {
        audioManager.channel->getPosition(&currentPosition, FMOD_TIMEUNIT_MS);
        float progress = float(currentPosition) / float(soundLength);       // Calculates progress done
        ...
        std::cout << ") " << int(progress * 100.0) << " %\r";           // Printing progress bar

        if (_kbhit()) {
            int key = _getch();
            if (key == 'H' || key == 'h') {
                cout << "------------------------------------Progress bar hidden----------------------------------------------" << endl;
                break;
            }
        }
        std::cout.flush();
