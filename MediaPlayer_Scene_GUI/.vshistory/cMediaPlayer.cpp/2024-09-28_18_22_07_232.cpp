...
void cMediaPlayer::StopMediaPlayer(GLFWwindow* window) {
    ...
    // Static variable to hold user decision
    static int decision = -1;  // -1 = No decision, 0 = No, 1 = Yes

    // Create ImGui window for the quit confirmation
    ImGui::Begin("Quit Confirmation");

    // Prompt the user
    ImGui::Text("You want to quit? Are you sure?");
    ImGui::Text("YES = Y, NO = N");

    // Add Yes and No buttons
    if (ImGui::Button("Yes")) {
        decision = 1;  // Set decision to Yes
    }
    ImGui::SameLine();  // Place No button on the same line
    if (ImGui::Button("No")) {
        decision = 0;  // Set decision to No
    }
    ...
    if (decision == 1) {
        ...
        ImGui::Text("\n----------------------------------------------Unloading Songs!--------------------------------------------");
        audioManager.unloadAssets();  // Unload the songs
        ListSongs();  // List the songs (optional based on your function implementation)
        ImGui::Text("\n----------------------------------------------BYE for now!--------------------------------------------");
        // Optionally close the media player
        StopMediaPlayer(window);  // Stop the media player window
    }
    else if (decision == 0) {
        // User selected No
        ImGui::Text("Continue listening...");
        RunMediaPlayer(window);  // Continue running the media player
    }

    // If an invalid choice is entered
    if (decision != -1 && decision != 0 && decision != 1) {
        ImGui::Text("Enter a valid choice!");
        StopMediaPlayer(window);  // Stop the media player for invalid input
    }
