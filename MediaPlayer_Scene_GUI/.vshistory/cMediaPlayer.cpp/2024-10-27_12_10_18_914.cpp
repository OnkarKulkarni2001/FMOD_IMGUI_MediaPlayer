...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    FMOD_VECTOR soundPosition = { 10.0f, 0.0f, 0.0f };
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Play selected scene")) {
            ...
            for (int index = 1; index <= channelGroupManager.sounds.size(); index++) {
                ...
                /*if (index == 2) {
                    ...
                    soundPosition = { x, 0.0, z };

                    if (channelGroupManager.channel->isPlaying(&channelGroupManager.bIsPlaying)) {
        ...
        channelGroupManager.m_ChannelGroup->getChannel(2, &channel2);
        if (channel2) {
            bool isPlaying = false;
            channel2->isPlaying(&isPlaying);

            // Only update position if the sound is playing
            if (isPlaying) {
                ...
                channel2->set3DAttributes(&soundPosition, nullptr);
                channelGroupManager.SetSoundPosition(soundPosition);
        ...
        ImGui::SliderFloat("Sound Position X", &soundPosition.x, -10.0f, 10.0f);
        
        ImGui::SliderFloat("Sound Position Z", &soundPosition.z, -10.0f, 10.0f);
