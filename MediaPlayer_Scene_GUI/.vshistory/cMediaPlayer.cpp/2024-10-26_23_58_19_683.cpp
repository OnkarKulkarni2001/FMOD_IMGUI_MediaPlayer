...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Play selected scene")) {
            ...
            float minDistance = 0.5f;   // Start attenuating close to the listener
            float maxDistance = 50.0f;  // Sound becomes inaudible at 50 units away
            channelGroupManager.channel->set3DMinMaxDistance(minDistance, maxDistance);

            for (int i = 0; i < 10; ++i) {
                listenerPosition.x += 25.0f;
                channelGroupManager.setListenerAttributes(listenerPosition, listenerForward, listenerUp);
