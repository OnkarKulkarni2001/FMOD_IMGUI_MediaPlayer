...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Play selected scene")) {
            channelGroupManager.m_ChannelGroup->getName(name, sizeof(name));
            if( name == "Scene 01: Forest Ambience(Lively)"){
                channelGroupManager.SetChannelGroupDSP(0.7f, 300.0f);
            }
            if (name == "Scene 02: Horror (Probably You Shouldn't hear this)") {
                channelGroupManager.SetChannelGroupDSP(0.2f, 3000.0f);
            }
            if (name == "Scene 03: Port Control (Communication)") {
                channelGroupManager.SetChannelGroupDSP(1.7f, 30.0f);
            }
            ...
            for (int index = 1; index <= channelGroupManager.sounds.size(); index++) {
                PlayAudio(index);
                if (index == 2) {
                    FMOD_VECTOR soundPosition = { 10.0, 0.0, 0.0 };
