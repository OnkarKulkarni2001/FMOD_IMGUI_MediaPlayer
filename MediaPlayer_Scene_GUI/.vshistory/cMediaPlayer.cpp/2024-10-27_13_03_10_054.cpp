...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        ImGui::Text("Welcome to the Media Player developed by Onkar Kulkarni! Below are some scenes you can play right away!\nI have added some distortion and echo as per requirements but you can set those as per your convenience too! Please read README for more details on how to configure sound for each scene!");
        ...
        if (ImGui::Button("Play selected scene")) {
            ...
            for (int index = 1; index <= channelGroupManager.sounds.size(); index++) {
                ...
                if (index == 2) {
                    ...
                    std::cout << "Channel for index 2 assigned: " << (index2Channel ? "Yes" : "No") << std::endl;
                }
            }
        }
        if (index2Channel) {
