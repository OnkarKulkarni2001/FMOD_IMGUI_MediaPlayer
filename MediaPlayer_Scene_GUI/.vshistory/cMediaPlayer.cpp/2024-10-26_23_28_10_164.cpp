    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Play selected scene")) {
            ...
            for (int index = 1; index <= channelGroupManager.sounds.size(); index++) {
                PlayAudio(index);
                if (index == 2) {
                    FMOD_VECTOR soundPosition = { 5.0, 3.0, 2.0 };
                    FMOD_VECTOR soundVelocity = { 1.0, 0.0, 0.0 };
                    channelGroupManager.channel->set3DAttributes(&soundPosition, &soundVelocity);
                }
            }

            for (int i = 0; i < 100; ++i) {
                // Optionally, update listener position to simulate movement
                listenerPosition.x += 0.1f;  // Move listener along the X axis
                channelGroupManager.setListenerAttributes(listenerPosition, listenerForward, listenerUp);

                channelGroupManager.updateSystem();  // Update FMOD
            }
        }
