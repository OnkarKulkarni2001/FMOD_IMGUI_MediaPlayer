...
void cChannelGroupManager::pause(int index, FMOD::Channel* channel) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
...
void cChannelGroupManager::resume(int index, FMOD::Channel* channel) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
...
void cChannelGroupManager::stop(int index, FMOD::ChannelGroup* channelGroup) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
    ...
    result = channelGroup->getChannel(index, &channel);

    if (result == FMOD_OK && channel) {
        channel->isPlaying(&bIsPlaying);
        if (bIsPlaying) {
            channel->stop();  // Stop the specific channel
        }
    }
    else {
        std::cout << "Failed to retrieve channel or channel is null at index: " << index << std::endl;
        FMODCheckError(result);
    }
}

void cChannelGroupManager::setPitch(float pitchValue, FMOD::Channel* channel) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
...
void cChannelGroupManager::setVolume(float volumeValue, FMOD::Channel* channel) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
...
void cChannelGroupManager::setPan(float panValue, FMOD::Channel* channel) {
    if (!m_Initialized) {
        cout << "FMOD not initialized" << endl;
        return;
    }
