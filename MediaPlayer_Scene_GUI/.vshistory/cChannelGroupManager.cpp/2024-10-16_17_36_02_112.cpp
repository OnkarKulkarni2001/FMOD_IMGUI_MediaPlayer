...
float distortionLevel = 0.0f; // Initialize distortion level
float dspChorus = 0.0f; // Example for another DSP parameter like gain
// Reference to DSP object, so it can be modified later
...
void cChannelGroupManager::SetChannelGroupDSP() {
    ...
    if (distortionDSP == nullptr) {
        // Create DSP once when the button is pressed
        ...
        // Sliders for adjusting DSP parameters if the DSP is already created
        ImGui::Text("Adjust DSP Parameters:");

        // Slider for Distortion Level
        ...
        // Slider for DSP Gain (example parameter)
        if (ImGui::SliderFloat("Chorus", &dspChorus, 0.0f, 1.0f)) {
            FMOD_RESULT result = distortionDSP->setParameterFloat(FMOD_DSP_TYPE_CHORUS, dspChorus); // Example using GAIN
            if (result != FMOD_OK) {
                snprintf(dspMessage, IM_ARRAYSIZE(dspMessage), "Error setting gain: %s", FMOD_ErrorString(result));
            }
        }
    }
