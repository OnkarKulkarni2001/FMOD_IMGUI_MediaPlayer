...
void cChannelGroupManager::UnloadAssets()
{
    ...
    if (choice == 1) {
        ...
        int index;
        std::cout << "Enter the index of the song to unload (1 to " << pSounds.size() << "): ";
        std::cin >> index;


        index -= 1;     // Converting user input to zero-based index

        if (index >= 0 && index < pSounds.size()) {
            pSounds[index]->release(); // Releasing the sound resource
            pSounds.erase(pSounds.begin() + index); // Removing from the vector
        ...
        pSounds.clear(); // Clearing the vector
...
void cChannelGroupManager::ApplyDSPToSound(int songIndex, FMOD::Channel* channel)
{
    if (!m_Initialized)
        return;
    ...
    result = m_ChannelGroup->getChannel(songIndex, &channel);
...
void cChannelGroupManager::PlayAudio(FMOD::ChannelGroup* channelGroup)
{
    if (!m_Initialized)
        return;
    ...
    result = fmodSystem->playSound(sound, m_ChannelGroup, false, &channel);
    ...
    char name[256];
    result = sound->getName(name, sizeof(name));
