...
float echoDelay = 50.0f;

FMOD::DSP* distortionDSP = nullptr;
FMOD::DSP* echoDSP = nullptr;

void cChannelGroupManager::setListenerAttributes(FMOD_VECTOR position, FMOD_VECTOR forward, FMOD_VECTOR up) {
    // Set listener's position and orientation
    fmodSystem->set3DListenerAttributes(0, &position, nullptr, &forward, &up);
}

void cChannelGroupManager::SetListenerPosition(FMOD_VECTOR newListenerPosition) {
    listenerPosition = newListenerPosition;
}

void cChannelGroupManager::SetSoundPosition(FMOD_VECTOR newSoundPosition) {
...
void cChannelGroupManager::LoadScene() {
    if (ImGui::Button("Scene 01: Forest Ambience (Lively)")) {
        FMOD_RESULT result = fmodSystem->createChannelGroup("Scene 01: Forest Ambience (Lively)", &m_ChannelGroup);
        echoDSP->setParameterFloat(FMOD_DSP_ECHO_DELAY, 0.0f);
        ...
        for (std::vector<FMOD::Sound*>::iterator::value_type& sound : sounds) {
            sound->release(); // Releasing each sound
        }
        sounds.clear(); // Clearing the vector

        loadAssets("Scene1");
        SetChannelGroupDSP(0.7f, 300.0f);
    }
    if (ImGui::Button("Scene 02: Horror (Probably You Shouldn't hear this)")) {
        FMOD_RESULT result = fmodSystem->createChannelGroup("Scene 02: Horror (Probably You Shouldn't hear this)", &m_ChannelGroup);
        echoDSP->setParameterFloat(FMOD_DSP_ECHO_DELAY, 0.0f);
        ...
        loadAssets("Scene2");
        SetChannelGroupDSP(0.2f, 300.0f);
    }
    if (ImGui::Button("Scene 03: Port Control (Communication)")) {
        FMOD_RESULT result = fmodSystem->createChannelGroup("Scene 03: Port Control (Communication)", &m_ChannelGroup);
        echoDSP->setParameterFloat(FMOD_DSP_ECHO_DELAY, 0.0f);
...
void cChannelGroupManager::SetChannelGroupDSP(float distortionLevel, float echo)
{
    if (!m_Initialized) {
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "System not initialized!");
        return;
    ...
    // Ensure DSPs are created
    if (!distortionDSP) {
        fmodSystem->createDSPByType(FMOD_DSP_TYPE_DISTORTION, &distortionDSP);
        distortionDSP->setParameterFloat(FMOD_DSP_DISTORTION_LEVEL, distortionLevel);
    }

    if (!echoDSP) {
        fmodSystem->createDSPByType(FMOD_DSP_TYPE_ECHO, &echoDSP);
        echoDSP->setParameterFloat(FMOD_DSP_ECHO_DELAY, echoDelay);
    }

    // Add DSPs to the channel group
    if (m_ChannelGroup) {
        m_ChannelGroup->addDSP(0, distortionDSP);
        m_ChannelGroup->addDSP(1, echoDSP);
    }
}
...
void cChannelGroupManager::PrintChannelGroupInfo() {
    ...
    distortionDSP->getParameterFloat(0, &distortionLevel, dspMessage, sizeof(dspMessage));

    float echo = 0.0f;
    echoDSP->getParameterFloat(0, &echo, dspMessage, sizeof(dspMessage));

    ImGui::Text("Channel Group Info:");
    ImGui::Text("Name: %s", name);
    ImGui::Text("Volume: %.2f", volume);
    ImGui::Text("Pitch: %.2f", pitch);
    ImGui::Text("Distortion Level: %.2f", distortionLevel);
    ImGui::Text("Echo: %.2f", echo);
    ...
    if (ImGui::Button("Load Songs")) {
        ...
        do {
            ...
            if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                ...
                FMOD::Sound* newSound;
                result = fmodSystem->createSound(std::string(filePath.begin(), filePath.end()).c_str(), FMOD_3D, 0, &newSound);
...
void cChannelGroupManager::play(int index, FMOD::Channel* channel) {
    if (!m_Initialized) {
...
void cChannelGroupManager::Update() {
    fmodSystem->update();
}
