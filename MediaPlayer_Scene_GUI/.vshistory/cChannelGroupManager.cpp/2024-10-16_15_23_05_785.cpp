...
void cChannelGroupManager::CreateChannelGroup(const char* channel_name) {
    if (!m_Initialized) {
        return;
    }
    ...
    if (ImGui::Button("Create Channel Group")) {
        ...
        if (strlen(channel_name) > 0) {
            ...
            if (result != FMOD_OK) {
                ...
                std::cout << "Channel group " << channel_name << " has been created!\n" << std::endl;
            }
        }
        else {
            std::cout << "Please enter a valid channel name.\n" << std::endl;
        }
    }
}
