...
void cChannelGroupManager::CreateChannelGroup() {
    ...
    static char channelGroupName[256] = "";
    ImGui::Text("Give a name to your channel group!");
    ImGui::InputText("Channel Group Name", channelGroupName, sizeof(channelGroupName));
    ...
    if (ImGui::Button("Create Channel Group")) {
        ...
        if (strlen(channelGroupName) > 0) {
            FMOD_RESULT result;
            result = fmodSystem->createChannelGroup(channelGroupName, &m_ChannelGroup);
            ...
            if (result != FMOD_OK) {
                ...
                char message[256];
                snprintf(message, sizeof(message), "Channel group '%s' has been created!\n", channelGroupName);
                ImGui::Text("%s", message);
                return;
            }
        }
        else {
            ImGui::Text("Please enter a valid channel name.\n");
        }
    }
}
...
void cChannelGroupManager::PrintChannelGroupInfoUI() {
