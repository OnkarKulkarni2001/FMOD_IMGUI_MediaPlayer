...
void cChannelGroupManager::SetChannelGroupDSP() {
    ...
    if (distortionDSP == nullptr) {
        ...
        if (ImGui::SliderFloat("Chorus", &dspEcho, 0.0f, 1.0f)) {
            FMOD_RESULT result = distortionDSP->setParameterFloat(FMOD_DSP_TYPE_ECHO, dspEcho); // Example using GAIN
            if (result != FMOD_OK) {
                snprintf(dspMessage, IM_ARRAYSIZE(dspMessage), "Error setting chorus: %s", FMOD_ErrorString(result));
            }
        }
    }
