...
void cAudioManager::setPitch() {
    ...
    if (channel) {
        ...
        if (bIsPlaying) {
            ...
            float normalPitch = channel->getPitch(&pitchValue);
            while (true) {
                if (_kbhit()) {
                    int key = _getch();

                    if (key == 'q') {
                        pitchValue = normalPitch + pitchValue + 0.1;
                        /*if (pitchValue > 1.0) {
                            pitchValue = 1.0 + normalPitch;
                        }*/
                        result = channel->setPitch(pitchValue);
                        cout << "New Pitch: " << pitchValue << endl;
                    }
                    if (key == 'e') {
                        pitchValue = pitchValue - normalPitch - 0.1;
                        /*if (pitchValue < -1.0) {
                            pitchValue = normalPitch - 1.0;
                        }*/
                        result = channel->setPitch(pitchValue);
                        cout << "New Pitch: " << pitchValue << endl;
                    }
