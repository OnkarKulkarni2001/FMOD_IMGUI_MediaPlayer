...
void cAudioManager::play(int index) {
    ...
    bool bIsPlaying = false; // Flag to check if sound is playing
    bool bShowStopConfirmation = false; // Flag for showing confirmation dialog
    int currentSoundIndex = -1; // Current sound index
    ...
    if (bIsPlaying) {
        if (ImGui::Button("Pause")) {
            pause(); // Your pause function
            bShowStopConfirmation = true; // Show confirmation dialog
        }
    }
    else {
        if (ImGui::Button("Play")) {
            int userInput = currentSoundIndex; // Replace with your logic to select sound
            result = fmodSystem->playSound(sounds[userInput], nullptr, false, &channel);
            if (result == FMOD_OK) {
                bIsPlaying = true;
                std::cout << "Enjoy listening..." << std::endl;
            }
            else {
                FMODCheckError(result);
            }
    ...
    // Confirmation dialog for stopping the current song
    if (bShowStopConfirmation) {
        ImGui::OpenPopup("Stop Confirmation");
    }

    if (ImGui::BeginPopupModal("Stop Confirmation", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::Text("Current song will stop if you want to play another song. Are you sure you want to stop this song?");
        if (ImGui::Button("Yes")) {
            stop(); // Your stop function
            bIsPlaying = false;
            char name[256];
            int selectedSongIndex = index;
            int userInput = index;
            bool isSelected = selectedSongIndex; // Check if this is the selected song
            if (ImGui::Selectable(name, isSelected)) {
                play(userInput); // Play the selected song
            }
            if (isSelected) {
                ImGui::SetItemDefaultFocus(); // Ensure the selected item remains in focus
            }
            currentSoundIndex = selectedSongIndex; // Set to the index of the new song you want to play
            result = fmodSystem->playSound(sounds[currentSoundIndex], nullptr, false, &channel);
            if (result == FMOD_OK) {
                bIsPlaying = true;
                std::cout << "Enjoy listening..." << std::endl;
            }
            ...
            bShowStopConfirmation = false; // Close confirmation dialog
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
        if (ImGui::Button("No")) {
            resume(); // Your resume function
            bShowStopConfirmation = false; // Close confirmation dialog
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
