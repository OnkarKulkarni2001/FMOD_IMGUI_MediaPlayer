...
void cAudioManager::setPitch() {
    ...
    if (channel) {
        ...
        if (bIsPlaying) {
            ...
            pitchValue = normalPitch + 1;
            cout << "Normal Pitch: " << normalPitch << endl;
            while (true) {
                if (_kbhit()) {
                    int key = _getch();
                    result = channel->setPitch(pitchValue);
                    normalPitch = channel->getPitch(&pitchValue);

                    if (key == 'q') {
                        normalPitch = pitchValue + 0.1;
                        if (normalPitch > 1.0) {
                            normalPitch = 1.0;
                        }
                        result = channel->setPitch(normalPitch);
                        cout << "New Pitch: " << normalPitch << endl;
                    }
                    if (key == 'e') {
                        normalPitch = pitchValue - 0.1;
                        if (normalPitch < -1.0) {
