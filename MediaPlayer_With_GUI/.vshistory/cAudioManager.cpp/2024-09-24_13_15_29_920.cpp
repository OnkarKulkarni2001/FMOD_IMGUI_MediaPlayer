...
void cAudioManager::play(int index) {
    ...
    if (result != FMOD_OK || !bIsPlaying) {
        // Start playing the selected song if nothing is playing
        result = fmodSystem->playSound(sounds[userInput], nullptr, false, &channel);
        if (result == FMOD_OK) {
            std::cout << "Playing song: " << userInput + 1 << std::endl;

            // Main loop to monitor song status
            do {
                fmodSystem->update();
                // Check if the song is still playing
                result = channel->isPlaying(&bIsPlaying);
                if (!bIsPlaying) {
                    std::cout << "Song " << userInput + 1 << " has finished playing." << std::endl;
                    break; // Exit the loop when the song finishes
                }
                Sleep(100); // Sleep to avoid busy-waiting (adjust the duration as needed)
            } while (bIsPlaying);
        ...
        // Handle case where another song is already playing
        pause();
        std::cout << "Current song will stop if you want to play another song. Are you sure you want to stop this song? Y/N" << std::endl;
        char decision = 'Y';
        std::cin >> decision;
        if (decision == 'Y' || decision == 'y') {
            ...
            std::cout << "Not a valid choice! The song will resume!" << std::endl;
            resume();
        }
    }
}
