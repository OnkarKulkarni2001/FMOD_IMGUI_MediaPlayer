...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        ImGui::NewFrame();
        ImGui::SetNextWindowSize(ImVec2(1280, 720));  // Set the window size to 400x300 pixels
        ...
        if (audioManager.channel) {
            ...
            audioManager.channel->getPosition(&position, FMOD_TIMEUNIT_MS); // Get current position

            unsigned int length;
            audioManager.sounds[selectedSongIndex]->getLength(&length, FMOD_TIMEUNIT_MS); // Get sound length
