...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Play Song")) {
            int indexOfSong = -1;
            ListSongs(); // Display song list to get the index
            ImGui::InputInt("Enter song number", &indexOfSong);
            if (indexOfSong >= 0 && indexOfSong < audioManager.sounds.size()) {
                PlayAudio(indexOfSong);
            }
            else {
                ImGui::Text("Invalid song number!");
            }
        }
...
void cMediaPlayer::ListSongs() {
    // Create a window for listing songs
    ImGui::Begin("List of Songs");

    // Iterate through the list of songs and display their names in the ImGui window
    char name[256];
    for (int index = 0; index < audioManager.sounds.size(); index++) {
        if (audioManager.sounds[index]) {
            ...
            if (result == FMOD_OK) {
                // Display the name of the sound in the ImGui window
                ImGui::Text("Song %d: %s", index + 1, name);
            }
            else {
                // Handle errors (e.g., if the sound name could not be retrieved)
                ImGui::Text("Error retrieving the name of sound %d", index + 1);
            }
        }
        else {
            ImGui::Text("Sound %d is null.", index + 1);
        }
    }
