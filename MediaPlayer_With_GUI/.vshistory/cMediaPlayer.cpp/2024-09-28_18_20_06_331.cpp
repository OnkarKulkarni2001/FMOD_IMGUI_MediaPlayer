...
void cMediaPlayer::StopMediaPlayer(GLFWwindow* window) {
    ...
    // Variable to store user's decision
    static int decision = -1;  // -1 for no decision, 0 for No, 1 for Yes
    ...
    // Create a window for quit confirmation
    ImGui::Begin("Quit Confirmation");

    // Text to prompt the user
    ImGui::Text("Do you want to quit? Are you sure?");
    ImGui::Text("YES = Y, NO = N");

    // Display YES and NO buttons
    if (ImGui::Button("Yes")) {
        decision = 1;  // User chose to quit
    }

    ImGui::SameLine();  // Place NO button on the same line
    if (ImGui::Button("No")) {
        decision = 0;  // User chose not to quit
    }
    ...
    if (decision == 1) {
        ...
        ImGui::Text("\n----------------------------------------------Unloading Songs!--------------------------------------------\n");
        audioManager.unloadAssets();
        ListSongs();  // Assuming this is an existing function to list the songs
        ImGui::Text("\n----------------------------------------------BYE for now!--------------------------------------------\n");

        // Optionally, close the media player or exit the app
        StopMediaPlayer(window);
        bIsRunning = false;  // Exit the main loop
    }
    else if (decision == 0) {
        // If user clicked 'No'
        ImGui::Text("Continue listening...");
        RunMediaPlayer(window);  // Continue the media player
    }
