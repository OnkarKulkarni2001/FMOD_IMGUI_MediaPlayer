...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("List Songs")) {
            ListSongs();
            int index = 0;
            const char* label = "List Of Songs!";
            for (index = 0, index < audioManager.sounds.size(); index++) {
                ImGui::ListBox(&label, audioManager.sounds[index], audioManager.sounds, audioManager.sounds.size(), -1);
            }
        }
