...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    bool bIsRunning = true;
    bool showAudioLoaderWindow = false;
    ...
    while (bIsRunning) {
        ...
        if (ImGui::Button("Load Songs")) {
            ...
            // Toggle the visibility of the audio loader window
            showAudioLoaderWindow = true;
            audioManager.loadAssets();  // Load the assets when the button is clicked
        }
        // If the flag is true, show the "Audio Loader" window
        if (showAudioLoaderWindow) {
            ImGui::Begin("Audio Loader");

            // You can display some text, progress, or song info here if needed
            ImGui::Text("Songs are loaded successfully!");

            // Provide a close button inside the window
            if (ImGui::Button("Close")) {
                showAudioLoaderWindow = false;  // Close the window when the "Close" button is clicked
            }
