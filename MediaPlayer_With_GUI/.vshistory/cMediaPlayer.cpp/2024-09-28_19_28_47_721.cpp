...
void cMediaPlayer::StopMediaPlayer(GLFWwindow* window) {
    ...
    bool bIsDecisionToQuit = true;
    // Main ImGui frame setup
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    ...
    if (ImGui::Button("Quit")) {
        audioManager.unloadAssets(); // Call this when you want to open the new window
    }

    // Conditionally display the new window based on the boolean flag
    if (bIsDecisionToQuit) {
        // Open a new ImGui window
        ImGui::Begin("Confirmation Window", &bIsDecisionToQuit);

        // Display buttons inside the window
        if (ImGui::Button("Yes")) {
            // Handle Yes button click
            std::cout << "Yes clicked!" << std::endl;
            bIsDecisionToQuit = false; // Close window after action
        }

        ImGui::SameLine(); // Place the "No" button on the same line
