...
void cAudioManager::setPitch() {
    ...
    if (channel) {
        ...
        if (bIsPlaying) {
            ...
            // Get current pitch
            channel->getPitch(&pitchValue);

            cout << "Press Shift to increase pitch or Ctrl to decrease pitch." << endl;

            // Wait for user to press a key
            while (true) {
                if (_kbhit()) {  // Check if a key is pressed
                    int key = _getch();  // Get the key press
                    if (key == 0 || key == 224) {
                        // Handle special keys (like arrow keys)
                        key = _getch();
                    }

                    // Shift key is detected as ASCII 0 or 224 followed by 42
                    if (key == 42) { // Shift key
                        pitchValue += 0.1f;
                        if (pitchValue > 2.0f) pitchValue = 2.0f;  // Limit max pitch to 2.0
                        cout << "Increasing pitch: " << pitchValue << endl;
                    }
                    // Ctrl key is detected as ASCII 0 or 224 followed by 29
                    else if (key == 29) {  // Ctrl key
                        pitchValue -= 0.1f;
                        if (pitchValue < 0.5f) pitchValue = 0.5f;  // Limit minimum pitch to 0.5
                        cout << "Decreasing pitch: " << pitchValue << endl;
                    }
                    // Exit loop when 'Esc' is pressed
                    else if (key == 27) {  // Escape key to exit the loop
                        cout << "Exiting pitch adjustment." << endl;
                        break;
                    }
                    ...
                    if (result == FMOD_OK) {
                        cout << "Pitch set successfully." << endl;
                    }
                    else {
                        cerr << "Error occurred while setting pitch: " << FMOD_ErrorString(result) << endl;
                    }
                }
            }
        }
