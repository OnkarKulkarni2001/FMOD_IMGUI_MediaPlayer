...
void cAudioManager::setPitch() {
    ...
    if (channel) {
        channel->isPlaying(&bIsPlaying);
        if (bIsPlaying) {
            cout << "Press left Shift button to increase pitch by 0.1 and left ctrl to decrease pitch by 0.1." << endl;
            ...
            while (true) {
                if (_kbhit()) {
                    int key = _getch();
                    if (key == 0 || key == 224) {
                        // Special keys (like arrow keys, function keys, etc.) are preceded by a 0 or 224
                        int specialKey = _getch();  // Get the special key code
                        cout << "Special key detected: " << specialKey << " (preceded by " << key << ")" << endl;
                    }
                    else {
                        cout << "ASCII code: " << key << " for key '" << char(key) << "'" << endl;
                    }
                    if (key == 42) {
                        pitchValue += 0.1;
                        result = channel->setPitch(pitchValue);
