...
void cAudioManager::play(int index) {
    if (index < 0 || index >= sounds.size() || !isdigit(index)) {
        std::cout << "Invalid index! Please enter a value between 0 and " << sounds.size() - 1 << "." << std::endl;
        return; // Early exit to prevent further issues
    }

    //if (!bIsPlaying) {
    //    cout << "Song is finished playing..." << endl;
    //}
    if (bIsPlaying) {
        pause();
