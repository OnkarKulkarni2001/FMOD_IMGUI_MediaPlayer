...
void cAudioManager::loadAssets() {
    static bool showAudioLoader = true;
    if (!m_Initialized) {
        ImGui::Text("FMOD not initialized");
        return;
    }

    // Create the Audio Loader window
    if (showAudioLoader) {
        ImGui::Begin("Audio Loader", &showAudioLoader); // This allows the window to be closed
        ...
        if (ImGui::Button("Load Songs")) {
            ...
            if (hFind == INVALID_HANDLE_VALUE) {
                ...
                FindClose(hFind); // Ensure the handle is closed
                return;
            }
