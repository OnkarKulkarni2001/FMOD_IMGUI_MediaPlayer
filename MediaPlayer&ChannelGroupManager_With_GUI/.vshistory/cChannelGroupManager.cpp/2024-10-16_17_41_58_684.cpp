...
void cChannelGroupManager::SetChannelGroupDSP() {
    ...
    // Setup for Reverb DSP
    if (reverbDSP == nullptr) {
        if (ImGui::Button("Set Reverb DSP")) {
            FMOD_RESULT result = fmodSystem->createDSPByType(FMOD_DSP_TYPE_SFXREVERB, &reverbDSP);
            if (result == FMOD_OK) {
                result = reverbDSP->setParameterFloat(FMOD_DSP_SFXREVERB_ROOMSIZE, reverbRoomSize);
                if (result == FMOD_OK) {
                    m_ChannelGroup->addDSP(0, reverbDSP);
                    snprintf(dspMessage, IM_ARRAYSIZE(dspMessage), "Reverb DSP has been set!");
                }
                else {
                    snprintf(dspMessage, IM_ARRAYSIZE(dspMessage), "Error: %s", FMOD_ErrorString(result));
                }
    ...
    if (reverbDSP) {
        ImGui::Text("Adjust Reverb DSP Parameters:");
        if (ImGui::SliderFloat("Reverb Room Size", &reverbRoomSize, 0.0f, 1.0f)) {
            FMOD_RESULT result = reverbDSP->setParameterFloat(FMOD_DSP_SFXREVERB_ROOMSIZE, reverbRoomSize);
            if (result != FMOD_OK) {
                snprintf(dspMessage, IM_ARRAYSIZE(dspMessage), "Error setting reverb room size: %s", FMOD_ErrorString(result));
            }
        }
    }
