...
void cChannelGroupManager::loadAssets(static char* folderPath)
{
    if (!m_Initialized) {
        ImGui::Text("FMOD not initialized");
        return;
    ...
    ImGui::InputText("Folder Path", folderPath, sizeof(folderPath));  // Taking Input for folder path

    ImGui::Text("Please wait for songs to load if your directory is bigger than usual...");
    // Button to trigger loading of assets
    if (ImGui::Button("Load Songs")) {

        std::string folderPath = std::string(folderPath);  // Converting input to std::string
