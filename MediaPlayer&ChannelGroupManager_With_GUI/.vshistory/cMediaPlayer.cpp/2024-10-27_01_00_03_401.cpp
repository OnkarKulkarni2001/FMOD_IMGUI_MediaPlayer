...
void cMediaPlayer::RunMediaPlayer(GLFWwindow* window) {
    ...
    while (bIsRunning) {
        ...
        if (channelGroupManager.m_ChannelGroup->isPlaying(&channelGroupManager.bIsPlaying)) {
            if (ImGui::SliderFloat("Listener Position X", &listenerPosition.x, 0.0f, 10.0f)) {
                // Set volume whenever the slider is moved
                channelGroupManager.SetListenerPosition(listenerPosition);
            }
            if (ImGui::SliderFloat("Listener Position Y", &listenerPosition.y, 0.0f, 10.0f)) {
                // Set volume whenever the slider is moved
                channelGroupManager.SetListenerPosition(listenerPosition);
            }
            if (ImGui::SliderFloat("Listener Position Z", &listenerPosition.z, 0.0f, 10.0f)) {
                // Set volume whenever the slider is moved
                channelGroupManager.SetListenerPosition(listenerPosition);
            }
            channelGroupManager.setListenerAttributes(listenerPosition, listenerForward, listenerUp);
