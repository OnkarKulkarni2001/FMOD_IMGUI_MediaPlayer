...
void cMediaPlayer::StopMediaPlayer(GLFWwindow* window) {
    ...
    static bool bIsDecisionToQuit = true;

    if (bIsDecisionToQuit) {
        // Start the ImGui frame (this should usually be done in the main loop, not here)
        ImGui_ImplOpenGL3_NewFrame();
        ImGui_ImplGlfw_NewFrame();
        ImGui::NewFrame();

        // Create the ImGui window for the quit confirmation
        ImGui::Begin("Not enjoying? Wanna quit? Are you sure?");

        // Button for quitting
        if (ImGui::Button("Yes")) {
            StopAudio();
            audioManager.unloadAssets();  // Unload songs
            bIsDecisionToQuit = false;    // Close the confirmation window
        ...
        // Swap buffers and poll events (usually done in the main loop)
        glfwSwapBuffers(window);
        glfwPollEvents();
    }
    else {
        // Keep running the media player if user chooses "No"
